...
from tumbler.apps.market_maker.strategies.market_ama_v0_strategy import AMAMakerV0Strategy
from tumbler.function.technique import PD_Technique


def run_minute():
    symbol, sllippage, bar_window, rate = "btc_usdt", 0, 30, 0
    ...
    engine.set_parameters(vt_symbol=vt_symbol, interval=Interval.MINUTE.value,
                          ...
                          slippage=sllippage, size=1, price_tick=0.000001, capital=0, end=datetime(2023, 3, 30, 9, 50),
    ...
    engine.add_strategy(AMAMakerV0Strategy, setting)
