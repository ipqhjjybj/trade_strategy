...
from tumbler.apps.cta_strategy.strategies.bolling_bandit_strategy import BollingerBanditStrategy 
...
def run():
    ...
    slippage=10,  size=1, price_tick=0.000001, capital = 0, end = datetime(2020,1,29,9,50), mode = BacktestingMode.BAR.value)
    ...
    # engine.set_parameters(vt_symbol="btc_usdt.BsINANCE", interval=Interval.MINUTE.value, start=datetime(2017,1,1) ,rate = 0,
    #     slippage=0.01,  size=1, price_tick=0.01, capital = 0, end = datetime(2019,12,30), mode = BacktestingMode.BAR.value)

    engine.add_strategy(BollingerBanditStrategy, setting)
    engine.load_data()
