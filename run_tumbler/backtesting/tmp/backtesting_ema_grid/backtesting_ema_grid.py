...
from tumbler.apps.cta_strategy.strategies.ema_grid_strategy import EmaGridStrategy
...
def run():
    ...
    engine.set_parameters(vt_symbol="btc_usdt.BINANCE", interval=Interval.MINUTE.value,
                          start=datetime(2017, 1, 1, 8, 10), rate=0,
                          slippage=0, size=1, price_tick=0.01, capital=0, end=datetime(2021, 8, 15, 9, 50),
                          mode=BacktestingMode.BAR.value)

    filename = "/Users/szh/git/personal_tumbler/run_tumbler/tools/produce_csv_data/.tumbler/fix_{}_min1.csv"\
        .format("btc_usdt")

    engine.add_strategy(EmaGridStrategy, setting)
