...
from tumbler.apps.market_maker.strategies.line2_strategy import Line2Strategy
...
def run():
    ...
    # engine.set_parameters(vt_symbol="btc_usdt.BINANCE", interval=Interval.MINUTE.value, start=datetime(2015,10,1,8,10) ,rate = 0,
    # slippage=0.01, size=1, price_tick=0.01, capital = 0, end = datetime(2019,12,29,9,50), mode = BacktestingMode.BAR.value)
    ...
    engine.add_strategy(Line2Strategy, setting["line_btc_usdt_binance"]["setting"])
    engine.load_data()
    engine.run_backtesting()
    df = engine.calculate_result()
    engine.calculate_statistics()
