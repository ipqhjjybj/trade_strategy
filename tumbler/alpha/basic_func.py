...
from tumbler.function.technique import IncStdAvg
...
def winsorize(x):
    ...
    sigma = x.std()
    m = x.mean()
    x[x > m + 3 * sigma] = m + 3 * sigma
    x[x < m - 3 * sigma] = m - 3 * sigma
    return x


def numpy_winsorize(x):
    """
    numpy 去极值
    """
    inc_v = IncStdAvg()
    ...
    for i in range(ll):
        v = x[i]
        m, sigma, _ = inc_v.incre_in_value(v)
        if v > m + 3 * sigma:
            v = m + 3 * sigma
        elif v < m - 3 * sigma:
            v = m - 3 * sigma
...
def my_deal(x):
    return numpy_standardize(numpy_winsorize(np.nan_to_num(np.array(x), 0)))
