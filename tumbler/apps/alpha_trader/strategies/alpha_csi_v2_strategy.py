...
class AlphaCsiV2Strategy(AlphaTemplate):
    '''
    V2 是直接采用内存内计算的方案，
    1.首先记载历史的 bar
    ...
    def __init__(self, mm_engine, strategy_name, settings):
        super(AlphaCsiV2Strategy, self).__init__(mm_engine, strategy_name, settings)
    ...
    def update_day_work(self):
        ...
        if self.time_read_coins_time.can_work() and 8 <= now.hour and self.bef_update_day != now.day:
            ...
            if all_ready:
                ...
                self.today_long_coins = ret[:self.keep_num]
                self.today_short_coins = ret[-self.keep_num:]
