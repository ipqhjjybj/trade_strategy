...
class AlphaCsiV4Strategy(AlphaTemplate):
    '''
    V4 是直接采用内存内计算的方案，
    1.在 V3 的基础上，增加指数过滤方案， 比如 btc_usdt 5日EMA线高于60日EMA线，则选择清空持仓
    2.在 V3 的基础上，增加数值筛选方案， 原先是只按照排序的原则，计算某个数值，然后排序取前面的几个， 现在要增加数值筛选条件
        比如:
            选的票要5日均线>=60日均线
            选的票20日涨幅>0
    ...
    index_symbol = "btc_usdt"
    index_func = None
    day_rank_func = None
    day_filter_func = None
    hour_func = None
    select_num = 15
    keep_num = 10
    ...
    def __init__(self, mm_engine, strategy_name, settings):
        super(AlphaCsiV4Strategy, self).__init__(mm_engine, strategy_name, settings)
    ...
    def update_day_work(self):
        ...
        if self.time_read_coins_time.can_work() and 8 <= now.hour and self.bef_update_day != now.day:
            ...
            if all_ready:
                ...
                index_vt_symbol = get_vt_key(self.index_symbol, self.contract_exchange)
                self.write_log(f"[update_day_work] not go to check index! index_vt_symbol:{index_vt_symbol}!")
                if index_vt_symbol in self.future_manager.get_all_vt_symbols():
                    symbol_exchange = self.symbol_exchanges.get(index_vt_symbol, None)
                    if symbol_exchange.is_inited():
                        is_index_ok = symbol_exchange.run_day_func(self.index_func)
                        if not is_index_ok:
                            self.write_log(f"[update_day_work] index_day_func is :{is_index_ok} ! return!")
                            return
                else:
                    self.write_log(f"[update_day_work] index_vt_symbol not in symbol list! failed!")
                    return
                ...
                for vt_symbol in self.future_manager.get_all_vt_symbols():
                    ...
                    if symbol_exchange and symbol_exchange.is_inited():
                        is_check_filter = symbol_exchange.run_day_func(self.day_filter_func)
                        if is_check_filter:
                            val = symbol_exchange.run_day_func(self.day_rank_func)
                            ret.append((val, vt_symbol))
                        else:
                            self.write_log(f"[update_day_work] symbol_exchange:{vt_symbol} is filtered!")
                    else:
