...
class InsConfig:
    def __init__(self, ip, port):
        # conn = qconnection.QConnection(ip, port, pandas=True)
        # conn.open()
        self.contractTable = {}
        # self.contractTable = conn('contract_factor')
    ...
    def generateCommFunc(self, symbol):
        ...
        commT = entry['commType'].decode('UTF-8')
    ...
    def generateSingleInsConfig(self, symbol):
        tbl = {'symbol': symbol}
        entry = self.contractTable.loc[symbol]
        tbl['commType'] = entry['commType'].decode('UTF-8')
...
class TickEngine:
    def __init__(self, fillParam=1.0, queueParam=1.0, debug=False, logfileName='', qip='127.0.0.1', qport=5001):
        ...
        self.insCfg = InsConfig(qip, qport)
