...
from tumbler.function import get_vt_key, load_json, save_json, get_from_vt_key
from tumbler.constant import Exchange, Direction
from tumbler.engine import BaseEngine
from tumbler.object import SubscribeRequest, TickData, MergeTickData, TradeDataLog
from tumbler.function.convert import PositionHolding
from tumbler.service.log_service import log_service_manager

from .template import CalendarSpreadTemplate

STOP_ORDER_PREFIX = "8btc_cross_future_stop_"

"""
跨期套利 策略引擎
"""
class CalendarSpreadEngine(BaseEngine):

    setting_filename = "calendar_spread_setting.json"

    def __init__(self, main_engine, event_engine):
        super(CalendarSpreadEngine, self).__init__(main_engine , event_engine, APP_NAME)
    ...
    def send_stop_order(self, strategy, symbol, exchange, direction, offset, price, volume, stop):
        ...
        self.put_strategy_event(stop_order)

        ret = []
        ret.append( [stop_order_id, copy(stop_order)])
        return ret
    ...
    def load_strategy_class(self):
        ...
        self.load_strategy_class_from_folder(path1, "tumbler.apps.calendar_spread.strategies")
    ...
    def load_strategy_class_from_module(self, module_name):
        ...
        try:
            ...
            for name in dir(module):
                ...
                if (isinstance(value, type) and issubclass(value, CalendarSpreadTemplate) and value is not CalendarSpreadTemplate):
    ...
    def init_strategy(self, strategy_name):
        ...
        for vt_symbol in strategy.vt_symbols_subscribe:
            ...
            if contract != None:
                ...
                if contract.exchange == Exchange.BITMEX.value:
                    position_key = contract.vt_symbol
                    bitmex_position = self.main_engine.get_position(position_key)
                    if bitmex_position:
                        self.position_dic[contract.vt_symbol].update_position(bitmex_position)
                else: 
