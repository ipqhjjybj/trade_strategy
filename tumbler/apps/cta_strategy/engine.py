...
from tumbler.apps.cta_strategy.base import (
    APP_NAME
)
from tumbler.function.convert import PositionHolding
from tumbler.service.log_service import log_service_manager

STOP_ORDER_PREFIX = "8btc_cta_stop_"


def get_symbol_bars(vt_symbol):
    if "OKEXS" in vt_symbol or "OKEX5" in vt_symbol:
        if "btc_usd" in vt_symbol:
            return "btc_usdt.BINANCE"
        if "eth_usd" in vt_symbol:
            return "eth_usdt.BINANCE"
    elif "HUOBIS" in vt_symbol:
        return vt_symbol.replace(".HUOBIS", ".BINANCE")
    elif "BITMEX" in vt_symbol:
        if "btc_usdt" in vt_symbol:
            return "btc_usdt.BINANCE"
    return vt_symbol
...
class CtaEngine(BaseEngine):
    ...
    setting_filename = "cta_setting.json"
    ...
    def load_strategy_class(self):
        ...
        """
        self.write_log("[load_strategy_class]")
        path1 = Path(__file__).parent.joinpath("strategies")
        self.load_strategy_class_from_folder(
            path1, "tumbler.apps.cta_strategy.strategies")

        path2 = Path.cwd().joinpath("strategies")
        self.load_strategy_class_from_folder(path2, "strategies")

        self.load_strategy_class_from_folder("/Users/shenzhuoheng/git/personal_tumbler/tumbler/apps/cta_strategy/strategies", "tumbler.apps.cta_strategy.strategies")

    def load_strategy_class_from_folder(self, path, module_name=""):
        """
        Load strategy class from certain folder.
        """
        self.write_log(f"[load_strategy_class_from_folder] path:{path}")
        for dirpath, dirnames, filenames in os.walk(str(path)):
            for filename in filenames:
                #self.write_log(f"load_strategy_class_from_folder filename:{filename}")
    ...
    def load_strategy_class_from_module(self, module_name):
        ...
        try:
            ...
            for name in dir(module):
                #self.write_log(f"[load_strategy_class_from_module] name:{name}")
    ...
    def load_strategy_setting(self, setting={}):
        ...
        self.write_log(f"[load_strategy_setting] strategy_config:{self.strategy_setting}")
        for strategy_name, strategy_config in self.strategy_setting.items():
            self.write_log(f"[load_strategy_setting] load strategy_name:{strategy_name}")
    ...
    def add_strategy(self, class_name, strategy_name, setting):
        ...
        self.write_log(f"all classes:{self.classes}")
