...
class CoinManager(object):
    def __init__(self, contract, strategy):
...
class AutoMarketStrategy(CtaTemplate):
    """
    这个策略 机械做市，每分钟发买单跟卖单， 每天8点平掉
    """
    author = "ipqhjjybj"
    class_name = "AutoMarketStrategy"
    ...
    def __init__(self, mm_engine, strategy_name, settings):
        super(AutoMarketStrategy, self).__init__(mm_engine, strategy_name, settings)
    ...
    def on_bar(self, bar):
        ...
        if bar.datetime.hour == 8 and 0 <= bar.datetime.minute <= 5:
            ...
            for i in range(1, 1+self.put_order_nums):
                list_orders = self.buy(self.symbol_pair, self.exchange, bar.close_price - i * self.fixed_spread_price,
                                       self.fixed_volume)
                for vt_order_id, order in list_orders:
                    self.limit_order_dict[vt_order_id] = order

                list_orders = self.sell(self.symbol_pair, self.exchange, bar.close_price + i * self.fixed_spread_price,
                                        self.fixed_volume)
                for vt_order_id, order in list_orders:
                    self.limit_order_dict[vt_order_id] = order
