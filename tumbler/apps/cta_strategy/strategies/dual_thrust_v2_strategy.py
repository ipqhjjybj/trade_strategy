...
class DualThrustV2Strategy(CtaTemplate):
    """
    这个策略 首先通过计算出每日波动幅度，然后波动突破的时候开多开空

    """
    author = "ipqhjjybj"
    class_name = "DualThrustV2Strategy"
    ...
    def __init__(self, mm_engine, strategy_name, settings):
        super(DualThrustV2Strategy, self).__init__(mm_engine, strategy_name, settings)
    ...
    def on_window_bar(self, bar):
        ...
        self.now_open_num = 0
        self.buy_line = bar.close_price + (bar.high_price - bar.low_price) * self.rate
        self.sell_line = bar.close_price - (bar.high_price - bar.low_price) * self.rate

        self.strategy_statics.new_pos(self.pos)

        if self.strategy_statics.bar_since_entry >= self.exit_days:
            if self.pos != 0:
