...
class FactorMultiStockStrategy(CtaTemplate):
    """
    横截面多因子策略
    1.
    用多因子对多只股票进行横截面排序,  买入排名最高的前N， 卖出排名最低的N个，进行对冲
    ...
    """
    author = "ipqhjjybj"
    class_name = "FactorMultiStrategy"
    ...
    def __init__(self, mm_engine, strategy_name, settings):
        super(FactorMultiStockStrategy, self).__init__(mm_engine, strategy_name, settings)
    ...
    def on_init(self):
        ...
        if not self.is_backtesting:
            ...
            self.load_server_bars(self.vt_symbols_subscribe, 1, interval=Interval.DAY.value,
                                  callback=self.on_window_bar)
