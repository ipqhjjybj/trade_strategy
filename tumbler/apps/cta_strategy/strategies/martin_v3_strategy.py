...
class MartinV3Strategy(CtaTemplate):
    """
    这个策略 回测蓝猫改版那个策略

    测试
    增加日线 ema 过滤条件， EMA5日 > 20日才做多单 或者 ema5 > ema20 4小时周期
    增加布林条件过滤， 比如突破上下轨时，除非有仓位，否则不挂单
    """
    author = "ipqhjjybj"
    class_name = "MartinV3Strategy"
    ...
    def __init__(self, mm_engine, strategy_name, settings):
        super(MartinV3Strategy, self).__init__(mm_engine, strategy_name, settings)
        ...
        self.hour_bg = BarGenerator(self.on_bar, window=4, on_window_bar=self.on_hour_window_bar,
                                    interval=Interval.HOUR.value)
        self.am = ArrayManager(40)

        self.hour_am = ArrayManager(25)

        self.long_direction = False
    ...
    def on_hour_window_bar(self, bar):
        am = self.hour_am
        am.update_bar(bar)

        self.write_log("[on_hour_window_bar] bar:{}".format(bar))
        if not am.inited:
            return

        fast_ma = Technique.x_average(am.close_array, 5)
        fast_ma0 = fast_ma[-1]

        slow_ma = Technique.x_average(am.close_array, 20)
        slow_ma0 = slow_ma[-1]
