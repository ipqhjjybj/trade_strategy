...
from tumbler.apps.cta_strategy.signals.cci_signal import CciSignal
from tumbler.apps.cta_strategy.signals.rsi_signal import RsiSignal
...
from tumbler.apps.cta_strategy.signals.livermore_signal import LivermoreSignal


class SignalStrategy(CtaTemplate):
    """
    ...
    def __init__(self, mm_engine, strategy_name, settings):
        super(SignalStrategy, self).__init__(mm_engine, strategy_name, settings)
        ...
        for signal_name in settings["signals"].keys():
            ...
            elif signal_name == "livermore":
                obj = LivermoreSignal(sig["param1"], sig["param2"])
            elif signal_name == "cci":
                obj = CciSignal(sig["cci_window"], sig["cci_level"])
            elif signal_name == "rsi":
                obj = RsiSignal(sig["rsi_window"], sig["rsi_level"])
    ...
    def on_window_bar(self, bar):
        ...
        self.target_pos = 0
        for signal_name, sig_obj in self.signal_dict.items():
            self.target_pos += sig_obj.get_signal_pos()
