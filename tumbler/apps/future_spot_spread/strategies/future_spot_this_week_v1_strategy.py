...
class FutureSpotSpreadV1Strategy(FutureSpotSpreadTemplate):
    '''
    ...
    def __init__(self, cs_engine, strategy_name, settings):
        super(FutureSpotSpreadV1Strategy, self).__init__(cs_engine, strategy_name, settings)
    ...
    def on_tick(self, tick):
        ...
        if tick.symbol == self.spot_symbol:
            self.spot_tick_decorder.update_tick(tick)
        elif tick.vt_symbol in self.contract_tick_dict.keys():
            self.contract_tick_dict[tick.vt_symbol].update_tick(tick)

        if tick.vt_symbol in self.order_module_dict.keys():
            self.order_module_dict[tick.vt_symbol].on_tick(tick)

        if self.time_update_future_manager.can_work():
            self.update_contracts()
            self.update_now_contract()

        if self.time_work.can_work():
            self.update_account()
            if self.compute_spread():
                # 判断价差计算成功才去计算仓位。。 换期有问题
                self.compute_target_positions()

                contract = self.future_manager.get_contract(get_vt_key(self.now_this_week_symbol, self.contract_exchange))
                #     根据合约判断是否能交易
                #     1.如果当前合约能交易，则进入交易时间
                #     2.如果当前合约进入不再交易的系统时间, 如果原先存在仓位，则平掉
                if not contract.is_in_clear_time(hours=0, minutes=30, seconds=0) and self.check_update_account_ok():
                    self.go_trade()
                else:
                    self.clear_symbol(self.now_this_week_symbol)
            else:
                self.write_log("[on_tick] compute_spread false!")
