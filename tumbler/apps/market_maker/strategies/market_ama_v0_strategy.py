...
class AMAMakerV0Strategy(MarketMakerTemplate):
    """
    单市场纯网格AMA 策略。
    ...
    def __init__(self, mm_engine, strategy_name, settings):
        super(AMAMakerV0Strategy, self).__init__(mm_engine, strategy_name, settings)
        ...
        pool_id, x, y = self.pool_setting
        self.pool_obj = PoolInfo(self, pool_id, x, y)
    ...
    def put_orders(self):
        ...
        inside_price = self.pool_obj.query_inside_price()

        need_send_order_list = []
        if inside_price < self.save_ticker.bid_prices[0]:
            ori_price = self.save_ticker.bid_prices[0]
            pool_id, direction, xx, yy = self.pool_obj.get_order(self.save_ticker.bid_prices[0])
        else:
            ori_price = self.save_ticker.ask_prices[0]
            pool_id, direction, xx, yy = self.pool_obj.get_order(self.save_ticker.ask_prices[0])
