...
class BinancefRestMarketApi(RestClient):

    def __init__(self, gateway):
        super(BinancefRestMarketApi, self).__init__()
    ...
    def connect(self, key, secret, proxy_host="", proxy_port=0):
        ...
        self.gateway.write_log("BinancefRestMarketApi start success!")
    ...
    def query_depth(self, symbol, depth=10):
        self.add_request(
            method="GET",
            path="/fapi/v1/depth?symbol={}&limit={}".format(change_system_format_to_binance_format(symbol), str(depth)),
