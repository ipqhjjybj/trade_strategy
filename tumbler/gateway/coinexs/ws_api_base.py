...
from .base import server_sign


class CoinexsWsApiBase(WebsocketClient):

    def __init__(self, gateway):
        super(CoinexsWsApiBase, self).__init__()
    ...
    def login(self):
        ...
        params = {
            ...
            "params": [
                self.key,
                server_sign(self.key, self.secret, nonce),
                nonce
            ],
    ...
    def on_packet(self, packet):
        ...
        if u_id is not None and int(u_id) == self.login_use_id:
            if "error" in packet.keys() and packet["error"] is not None:
                log_service_manager.write_log(packet)
            else:
                self.on_login()
        else:
            self.on_data(packet)
