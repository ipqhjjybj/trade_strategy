...
from .rest_market_api import HuobifRestMarketApi
from .rest_trade_api import HuobifRestTradeApi
from .ws_market_api import HuobifWsMarketApi
from .ws_trade_api import HuobifWsTradeApi
...
class HuobifGateway(BaseGateway):
    ...
    exchanges = [Exchange.HUOBIF.value]

    def __init__(self, event_engine):
        """Constructor"""
        super(HuobifGateway, self).__init__(event_engine, Exchange.HUOBIF.value)
    ...
    def unsubscribe(self, req):
        if self.rest_market_api:
            self.rest_market_api.unsubscribe(req)
        if self.rest_trade_api:
            self.rest_trade_api.unsubscribe(req)
        if self.ws_market_api:
            self.ws_market_api.unsubscribe(req)
        if self.ws_trade_api:
            self.ws_trade_api.unsubscribe(req)
