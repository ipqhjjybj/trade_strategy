...
class HuobifRestMarketApi(RestClient):
    """
    HUOBIU REST API
    """

    def __init__(self, gateway):
        super(HuobifRestMarketApi, self).__init__()
    ...
    def connect(self, key="", secret="", proxy_host="", proxy_port=0):
        ...
        self.gateway.write_log("HuobiuRestMarketApi start success!")
    ...
    def unsubscribe(self, req):
        symbol = req.symbol
        if symbol in self.all_symbols_set:
            if symbol in self.ticks.keys():
                self.ticks.pop(symbol)

            for q_req in self.all_request_list:
                if symbol == q_req.symbol:
                    self.all_request_list.remove(q_req)
                    break
            self.all_symbols_set.remove(symbol)

    def query_depth(self, symbol, u_type="step6"):
        self.add_request(
            method="GET",
            path="/market/depth?symbol={}&type={}".
