...
class HuobifWsMarketApi(HuobiWsApiBase):
    def __init__(self, gateway, ws_market_trade=False, ori_market_msg=False):
        super(HuobifWsMarketApi, self).__init__(gateway)
    ...
    def on_connected(self):
        self.gateway.write_log("HuobifWsMarketApi connected success!")

        for symbol in self.set_all_symbols:
            s = SubscribeRequest()
            s.symbol = symbol
            s.exchange = Exchange.HUOBIF.value
    ...
    def subscribe(self, req):
        ...
        tick.exchange = Exchange.HUOBIF.value
        ...
        self.gateway.write_log("subscribe req:{}".format(req))
        self.send_packet(req)

        # Subscribe to market detail update
        # self.req_id += 1
        # req = {
        #     "sub": "market.{}.detail".format(get_huobi_future_system_format_symbol(req.symbol)),
        #     "id": str(self.req_id)
        # }
        # self.send_packet(req)
    ...
    def unsubscribe(self, req):
        symbol = req.symbol
        self.set_all_symbols.remove(symbol)

        self.gateway.write_log(f"[unsubscribe] HuobifWsMarketApi {symbol}")
        # if symbol in self.ticks.keys():
        #     self.ticks.pop(symbol)
    ...
    def on_market_depth(self, data):
        """行情深度推送 """
        #print("[on_market_depth] data:{}".format(data))
        ...
        if "bids" in data["tick"].keys() and "asks" in data["tick"].keys():
            simplify_tick(tick, data["tick"]["bids"], data["tick"]["asks"])

            #print("[on_market_depth] tick:{}".format(tick.__dict__))
    ...
    def on_trade_detail(self, data):
        ...
        trade.symbol = symbol
        trade.exchange = Exchange.HUOBIF.value
