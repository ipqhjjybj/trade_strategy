...
class HuobifWsTradeApi(HuobiWsApiBase):
    def __init__(self, gateway):
        super(HuobifWsTradeApi, self).__init__(gateway)
    ...
    def subscribe(self, req):
        ...
        req = {
            "op": "sub",
            "cid": str(self.req_id),
            "topic": "orders.{}".format(get_huobi_future_ws_system_format_symbol(req.symbol))
        }
        self.send_packet(req)

    def unsubscribe(self, req):
        if req:
            self.gateway.write_log(f"[unsubscribe] HuobifWsTradeApi req:{req.__dict__}")
            self.set_all_symbols.remove(req.symbol)
        else:
            self.gateway.write_log(f"[unsubscribe] HuobifWsTradeApi req is None!")
    ...
    def on_connected(self):
        self.gateway.write_log("HuobifWsTradeApi connect success!")
        self.login()

    def on_login(self):
        self.gateway.write_log("HuobifWsTradeApi login success!")
