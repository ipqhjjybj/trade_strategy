...
from .base import STATUS_HUOBIS2VT


class HuobisWsTradeApi(HuobiWsApiBase):
    def __init__(self, gateway):
        super(HuobisWsTradeApi, self).__init__(gateway)
    ...
    def subscribe(self, req):
        ...
        req = {
            "op": "sub",
            "cid": str(self.req_id),
            "topic": "orders.{}".format(get_huobi_future_system_format_symbol(req.symbol))
        }
    ...
    def on_connected(self):
        self.gateway.write_log("HuobisWsTradeApi connect success!")
        self.login()

    def on_login(self):
        self.gateway.write_log("HuobisWsTradeApi login success!")

        for symbol in self.set_all_symbols:
            s = SubscribeRequest()
            s.symbol = symbol
            s.exchange = Exchange.HUOBIS.value
    ...
    def on_order(self, data):
        ...
        traded_volume = float(data["trade_volume"]) - order.traded
        # Push order event
        if traded_volume > 0:
        ...
        if float(data["trade_volume"]) > 0 and not order.is_active():
            for trade_dic in data["trade"]:
                ...
                trade.exchange = Exchange.HUOBIS.value
