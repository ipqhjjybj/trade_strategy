...
from tumbler.event import EVENT_SENDER_ORDER
...
from .rest_market_api import OkexfRestMarketApi
from .rest_trade_api import OkexfRestTradeApi
from .ws_market_api import OkexfWsMarketApi
from .ws_trade_api import OkexfWsTradeApi
...
class OkexfGateway(BaseGateway):
    ...
    exchanges = [Exchange.OKEXF.value]

    def __init__(self, event_engine):
        """Constructor"""
        super(OkexfGateway, self).__init__(event_engine, Exchange.OKEXF.value)
    ...
    def init_query(self):
        ...
        if self.run_mode in [RunMode.QUERY.value]:
            self.event_engine.register(EVENT_SENDER_ORDER, self.process_sender_order)

    def on_order(self, order):
        self.orders[order.order_id] = order
        super(OkexfGateway, self).on_order(order)
