...
from tumbler.record.defi.util import str_to_addr, validate_address, load_contract_erc20, addr_to_str
from tumbler.record.defi.constants import ETH_ADDRESS, CROSS_ETH_RECEIVER_ADDRESS, CROSS_USDT_ERC20_RECEIVER_ADDRESS
...
class EthClient(object):
    def __init__(
        ...
        if web3:
            ...
            self.provider = provider or config["ETH_PROVIDER"]
            self.w3 = Web3(
    ...
    @staticmethod
    def get_client_from_keystore(obj, keystore_file_path, password='', net=EthNet.MainNet.value):
        with open(keystore_file_path) as keyfile:
            encrypted_key = keyfile.read()
            private_key = w3.eth.account.decrypt(encrypted_key, password)
            address = eth_account.Account.from_key(private_key).address
            return obj(address, private_key, net)

    @staticmethod
    def get_eth_client_from_keystore(keystore_file_path, password='Shabi86458043.', net=EthNet.MainNet.value):
        return EthClient.get_client_from_keystore(EthClient, keystore_file_path, password, net)
    ...
    def transfer(self, to_address: str, amount: float, token_symbol):
        qty = Wei(int(amount * (10 ** get_symbol_2_decimal_dict(token_symbol.upper()))))
