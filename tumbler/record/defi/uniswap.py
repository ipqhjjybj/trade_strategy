...
from tumbler.record.defi.eth_client import EthClient
...
class Uniswap(EthClient):
    ...
    def _get_eth_token_input_price(self, token: AddressLike, qty: Wei, fee: int) -> Wei:
        ...
        if self.version == 1:
            ...
            log_service_manager.write_log(f"[_get_eth_token_input_price] version:2, qty:{qty}!")
            log_service_manager.write_log(f"[_get_eth_token_input_price] get_weth_address:{self.get_weth_address()}")
            log_service_manager.write_log(f"[_get_eth_token_input_price] token:{token}")
