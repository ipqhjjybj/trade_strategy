...
from tumbler.gateway.huobiu.base import REST_TRADE_HOST, get_huobi_future_system_format_symbol
from tumbler.gateway.huobiu.base import parse_order_info, parse_contract_info, parse_account_info
from tumbler.gateway.huobiu.base import parse_position_holding

from .huobi_client import HuobiClient


class HuobiuClient(HuobiClient):
    def __init__(self, _apikey, _secret_key, proxy_host="", proxy_port=0, account_id=None):
        super(HuobiuClient, self).__init__(_apikey, _secret_key)
    ...
    def get_exchange_info(self):
        ...
        if data and "data" in data.keys():
            for d in data["data"]:
                contract = parse_contract_info(d, Exchange.HUOBIU.value)
                ret.append(contract)
        return ret
    ...
    def get_ticker(self, symbol, depth=20, u_type="step0"):
        data = self.wrap_request("GET",
                                 "/linear-swap-ex/market/depth?contract_code={}&type={}"
                                 .format(get_huobi_future_system_format_symbol(symbol), u_type, str(depth)))
        tick = TickData()
        tick.symbol = symbol
        tick.vt_symbol = get_vt_key(symbol, Exchange.HUOBIU.value)
    ...
    def get_accounts(self):
        return self.wrap_request(method="POST", path="/linear-swap-api/v1/swap_cross_account_info")

    def get_positions(self):
        return self.wrap_request(method="POST", path="/linear-swap-api/v1/swap_cross_position_info")
    ...
    def get_position_info(self):
        ...
        position_list = []
        for d in data["data"]:
            pos = parse_position_holding(d, gateway_name=self.get_exchange_name())
