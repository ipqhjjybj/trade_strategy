...
class UniswapClient(Uniswap):
    ...
    def __init__(self, address, private_key, network=EthNet.MainNet.value, provider="", version=2):
        ...
        super(UniswapClient, self).__init__(address=address, private_key=private_key,
                                            network=network,
                                            provider=provider, version=version)

    @staticmethod
    def get_eth_client_from_keystore(keystore_file_path, password='Shabi86458043.', net=EthNet.MainNet.value):
        return UniswapClient.get_client_from_keystore(UniswapClient, keystore_file_path, password, net)
